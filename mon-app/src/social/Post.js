    ...
    return (
      ...
      <div className="card-post">
        <div className="header-card">
          ...
          >
            <Delete className="trash" refreshPosts={refreshPosts}  idPost={post.id} />
        ...
        <div className="icon-action">
          <i className="fa fa-comment comment" id={post.id}  onClick={commentModals} ></i>
          <Like idPost={post}/>
        </div>
        <div className="com-post">{post.message}</div>
        <div>
          <a className="date-post" tabIndex="0">
            {formatDistance(subDays(new Date(post.date), 0), new Date(), { addSuffix: true })}
          </a>
        </div>
      </div>
